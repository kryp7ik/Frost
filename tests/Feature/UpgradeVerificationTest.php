 ...
 * the Laravel 5.2 → 11.x upgrade per official upgrade guides.
 */
class UpgradeVerificationTest extends TestCase
{
    ...
    public function test_storage_disk_operations_succeed()
    {
        ...
        Storage::disk('local')->put('test.txt', 'Hello Laravel 11');

        Storage::disk('local')->assertExists('test.txt');
        $this->assertEquals('Hello Laravel 11', Storage::disk('local')->get('test.txt'));
    ...
    public function test_laravel_version_is_11x()
    {
        $this->assertStringStartsWith('11.', app()->version());
    }
}
